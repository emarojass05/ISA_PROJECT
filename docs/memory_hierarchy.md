...
La especificación del proyecto solicita simular una memoria externa más lenta que el procesador. La implementación actual modela este comportamiento mediante una latencia fija de 25 ciclos en el mismo dominio de reloj de simulación. Esto reproduce el efecto observable para el pipeline: stalls prolongados y penalización por miss. Como mejora futura, el mismo protocolo puede extenderse a dos dominios de reloj explícitos.
...
Su función es recibir líneas sucias expulsadas de L2 y drenarlas hacia memoria principal.

| Propiedad        | Descripción                                      |
| ---------------- | ------------------------------------------------ |
| Estructura       | FIFO de líneas de caché                          |
...
| Observabilidad   | Expone `empty` para el testbench                 |
...
En este caso se accede a memoria principal y se paga la latencia de 25 ciclos definida en `main_mem_model.sv`.
...
memory_penalty ≈ 25 ciclos + ciclos de llenado/write-back
```
...
- El modelo de memoria principal usa latencia fija de 25 ciclos en el mismo reloj de simulación. Esto reproduce el efecto de memoria lenta, aunque no modela un cruce explícito de dominios de reloj.
