...
# ── site-packages del usuario (por si graphviz está en ~/.local) ──────────
...
try:
    ...
    print("[ERROR] Falta la librería 'graphviz'.")
...
def compile_to_ir(source_file: str, opt_level: OptimizationLevel):
    ...
    try:
        ...
        print(f"[ERROR] Error semántico: {exc}")
        sys.exit(1)
...
def render_cfg(ir_program, out_dir: Path, fmt: str, view: bool, opt_label: str) -> list:
    ...
    for ir_func in ir_program.functions:
        ...
        print(f"  [OK] {ir_func.name}: {n_blocks} bloques, {n_edges} aristas  →  {final_path}")

    return rendered


# ── Modo comparación: O0 vs O2, HTML lado a lado ──────────────────────────
def render_compare(source_file: str, out_dir: Path, fmt: str, compare_with: str = "O2"):
    ...
    for level, label in compare_levels:
        ...
        results[label] = cfgs
        print(f"  {label}: {len(cfgs)} función(es) procesadas")
    ...
    for fn in results["O0"]:
        ...
        sections += f"""
        <section>
          <h2>función: <code>{fn}</code></h2>
          <table class="diff">
            <tr><th>Métrica</th><th>O0 (sin opt.)</th><th>{level_b} (con opt.)</th><th>Δ</th></tr>
            <tr><td>Bloques básicos</td><td>{o0.get('blocks','—')}</td><td>{o2.get('blocks','—')}</td><td>{fmt_delta(db)}</td></tr>
            <tr><td>Aristas (edges)</td><td>{o0.get('edges','—')}</td><td>{o2.get('edges','—')}</td><td>{fmt_delta(de)}</td></tr>
            <tr><td>Instrucciones IR</td><td>{o0.get('instrs','—')}</td><td>{o2.get('instrs','—')}</td><td>{fmt_delta(di)}</td></tr>
          </table>
          <div class="graphs">
            <div class="graph-box">
              <h3>O0 — Sin optimizaciones</h3>
              {o0.get('svg','<p>(no disponible)</p>')}
            </div>
            <div class="graph-box">
              <h3>{level_b} — Con optimizaciones</h3>
              {o2.get('svg','<p>(no disponible)</p>')}
            </div>
          </div>
        </section>"""
...
<body>
  <h1>CFG — Comparación de Optimizaciones</h1>
  <p class="subtitle">Archivo: <strong>{source_file}</strong> &nbsp;|&nbsp;
     Generado por <code>visualize_cfg.py --compare</code></p>
  {sections}
</body>
</html>"""
...
def main():
    ...
    ap.add_argument("source",    help="Archivo fuente .fr")
    ap.add_argument("--O1",      action="store_true", help="Optimizar con O1")
    ap.add_argument("--O2",      action="store_true", help="Optimizar con O2")
    ap.add_argument("--compare", action="store_true",
                    help="Generar comparación O0 vs O2 (o O1 con --O1) en HTML lado a lado")
    ...
    ap.add_argument("--view",    action="store_true",
                    help="Abrir imágenes al terminar")
    args = ap.parse_args()

    source_file = args.source
    if not Path(source_file).exists():
        print(f"[ERROR] Archivo no encontrado: {source_file}")
        sys.exit(1)
    ...
    print(f"  Formato : {args.fmt}")
    print(f"  Salida  : {out_dir}/")

    if args.compare:
        cw = "O1" if args.O1 else "O2"
        print(f"  Modo    : comparación O0 vs {cw}\n")
